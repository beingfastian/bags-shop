      ...
      <div className="px-6 sm:px-10 md:px-32 mt-10 overflow-x-auto">
        <table className="w-full text-left border-collapse rounded-[5px] overflow-hidden min-w-[800px]">
          ...
          {isPending ? (
            ...
            <tbody>
              {data?.items?.map((item) => {
                // Calculate discounted price for individual item
                const variantDiscount = item?.Variant?.discount || 0;
                const itemPrice = item?.price || 0;
                const discountedItemPrice = variantDiscount > 0 
                  ? Math.floor(itemPrice * (1 - variantDiscount / 100))
                  : itemPrice;
                
                // Calculate discounted total price
                const totalPrice = item?.total_price || 0;
                const discountedTotalPrice = variantDiscount > 0 
                  ? Math.floor(totalPrice * (1 - variantDiscount / 100))
                  : totalPrice;

                return item?.id === seleted && (isDeleting || isUpdating) ? (
                  <tr key={item.id}>
                    <td colSpan={5}>
                      <div
                        style={rowHeight ? { height: `${rowHeight}px` } : {}}
