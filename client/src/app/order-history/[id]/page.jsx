      ...
      <div className="w-full px-6 py-4">
        ...
        {order && (
          <Invoice
            ...
            items={
              order?.OrderItems?.map((item) => {
                // Ensure integer pricing for invoice display
                const unitPrice = Math.floor(item.unit_price || 0);
                ...
                return {
                  ...
                  price: unitPrice,
                  image: item.Variant?.image || '',
                  // Add total price for this item (quantity * unit_price)
                  total: Math.floor((item.quantity || 0) * unitPrice),
                };
              }) || []
            }
            companyName="Maaoz Official Store"
